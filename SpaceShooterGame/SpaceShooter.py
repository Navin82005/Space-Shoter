...
try:
                ...
                if collcon == False and powerupchannel2.get_busy() == 1:
                    if shimstate == True:
                        if shieldcnt >= len(shieldpower)-1:
                            shieldcnt = 0
                        shieldcnt += 0.1
                        display_surface.blit(shieldpower[int(shieldcnt)], (x+16, y+16))
    ...
    def bultfir():
        ...
        if not cor:
            ...
            if y <= width-100:
                y += 5
        by -= 10
        bullet = pygame.image.load(r"C:\Program Files (x86)\PandaGames\SpaceShooter\images\bullet.png")
    ...
    def enemyfire():
        ...
        if score%5 == 0 and game == False and estate == False and score != 0:
            ...
            bullete = pygame.image.load(r"C:\Program Files (x86)\PandaGames\SpaceShooter\images\ebullet.png")
    ...
    def bgsplay():
        ...
        if not voice.get_busy():
            sound = pygame.mixer.Sound(r"C:\Program Files (x86)\PandaGames\SpaceShooter\sounds\flying-saucer-26373.wav")
    ...
    while run:
        ...
        pygame.draw.rect(display_surface, gamecolor, pygame.Rect(0, 0, height, width), 10)
        ...
        if streak < 3:
            ...
            display_surface.blit(txt, ((height/2)- txt.get_width() // 2, (width/2) - (txt.get_height() // 2) - 50))
            display_surface.blit(scoretxt1, ((height/2)- scoretxt1.get_width() // 2, (width/2) - scoretxt1.get_height() // 2 + 10))
            display_surface.blit(scoretxt2, ((height/2)- scoretxt2.get_width() // 2, (width/2) - scoretxt2.get_height() // 2 + 50))
            replaytxt = scorefnt.render("Press enter to play again | esc to exit", True, (0, 255, 255))
            display_surface.blit(replaytxt, ((height/2) - replaytxt.get_width() // 2, (width) - replaytxt.get_height() - 20))
        ...
        if not game:
            ...
            highscoretxt = scorefnt1.render(f"Highscore: {highscore}", True, (0, 255, 255))
            display_surface.blit(highscoretxt, ((height//2)-highscoretxt.get_width()//2, 20))

            if streak == 0:
                j = 32
                for i in range(3):
                    life = pygame.image.load(r"C:\Program Files (x86)\PandaGames\SpaceShooter\images\life.png")
                    display_surface.blit(life, (height-streaktxt.get_width()-80+j, 25))
                    j+=32
            if streak == 1:
                j = 32
                for i in range(2):
                    ...
                    display_surface.blit(life, (height-streaktxt.get_width()-80+j, 25))
                    j+=32
                life = pygame.image.load(r"C:\Program Files (x86)\PandaGames\SpaceShooter\images\life-dead.png")
                display_surface.blit(life, (height-streaktxt.get_width()-80+j, 25))
            if streak == 2:
                j = 32
                for i in range(1):
                ...
                for i in range(2):
                    ...
                    display_surface.blit(life, (height-streaktxt.get_width()-80+j, 25))
                    j+=32
            pygame.draw.rect(display_surface, gamecolor, pygame.Rect(0, width-10, 1000, 20))
